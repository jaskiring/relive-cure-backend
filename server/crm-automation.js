...
async function processLead(lead) {
  ...
  try {
    ...
    // This is the primary data carrier for the sales rep on Refrens.
    // Everything the bot collected goes here so the rep has full context.
    const eyePowerDisplay = lead.eye_power || lead.eye_power_numeric || 'Not shared';
    const interests = [
      lead.interest_cost && '💰 Asked about cost',
      lead.interest_recovery && '🔄 Asked about recovery',
      lead.concern_pain && '😰 Worried about pain',
      lead.concern_safety && '🛡️ Worried about safety',
      lead.concern_power && '👓 Asked about eye power',
      lead.request_call && '📞 Wants a callback',
    ].filter(Boolean);
    ...
    const notes = [
      ...
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
      ``,
      `Eye Power:      ${eyePowerDisplay}`,
      lead.eye_power_numeric ? `Power (numeric): ${lead.eye_power_numeric}` : '',
      lead.power_stability ? `Power Stable:   ${lead.power_stability}` : '',
      `Insurance:      ${lead.insurance || 'Not shared'}`,
      `Timeline:       ${lead.timeline || 'Not shared'}`,
      ``,
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
      `🎯 INTENT & SIGNALS`,
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
      ``,
      `Intent Level:   ${(lead.intent_level || 'N/A').toUpperCase()}`,
      `Intent Score:   ${lead.intent_score || 0}/100`,
      `Urgency:        ${lead.urgency_level || 'low'}`,
      `Wants Call:     ${lead.request_call ? '✅ YES' : 'No'}`,
      lead.callback_source ? `Callback From:  ${lead.callback_source}` : '',
      ``,
      interests.length > 0 ? `Signals:` : '',
      ...interests.map(s => `  ${s}`),
      ``,
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
      `💬 CONVERSATION`,
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
      ``,
      `Messages:       ${lead.message_count || 'N/A'}`,
      `Bot State:      ${lead.current_flow_state || 'N/A'}`,
      `Params Done:    ${lead.parameters_completed || 0}/5`,
      `Assignee:       ${lead.assignee || 'Unassigned'}`,
      `Source:         ${lead.source || 'whatsapp_bot'}`,
      ``,
      `Last Message:   "${(lead.last_user_message || 'N/A').substring(0, 300)}"`,
      ``,
      lead.user_questions ? `Bot Notes:      ${lead.user_questions.substring(0, 500)}` : '',
